...
class ProcessCWL(BaseCWL):
    ...
    def _insert_input_params(self, input_filename=None):
        ...
        if self.app.stage_in_param is not None:
            ...
            if input_filename is None:
                input_filename = ""
            elif len(input_filename) != 0 and input_filename[0] != "/":
                # Make sure if input_filename has a path seperator if not already given
            ...
            self.process_cwl['arguments'] += [
                ...
                f'$(inputs.input.path){input_filename}'
            ]
    ...
    def _insert_output_params(self):
        ...
        if self.app.stage_out_param is not None:
            ...
            if not re.search('runtime.outdir', stage_out_process_dir):
                raise ProcessError(f"The process CWL template outputs/output path needs to contain $(runtime.outdir) in the path")
...
class BundledDataStagingCWL(ProcessCWL):
    ...
    def generate_all(self, outdir, **kwargs):
        ...
        generated_files = super().generate_all(outdir, **kwargs)
    ...
    def _insert_input_params(self):
        "Overrides ProcessCWL implementation to use old behavior of specifying a file instead of directory for stage-in"

        # We construct the stage in argument to point to the download directory 
        # with the filename mentioned in the stage_in CWL file. This replicates the old
        # monolithic application generation behavior.
        #
        # If we change the filename name in the stage in CWL template it is reflected here.
        input_dict = self.process_cwl['inputs']

        if self.app.stage_in_param is not None:
            stage_in_collection_filename = self.stage_in_cwl['outputs']['stage_in_collection_file']['outputBinding']['glob']

            super()._insert_input_params(stage_in_collection_filename)
